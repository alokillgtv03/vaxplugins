...
function rawJS(stream) { 
  return ` (function () { 
    ...
    var GAS_BASE_URL = 'https://script.google.com/macros/s/AKfycbx.../exec'; // Thay URL GAS của bạn vào đây
    ...
    // === HÀM KIỂM TRA API / VALIDATE URL BỔ SUNG ===
    ...
    function injectStyles() { 
      try { 
        ...
        style.textContent = 'html, body { margin: 0 !important; padding: 0 !important; width: 100vw !important; height: 100vh !important; overflow: hidden !important; background: #000 !important; }' + 
    ...
    function showLoadingScreen(msg) { 
      try { 
        ...
        if (!loadingDiv) { 
          ...
          loadingDiv.style.cssText = 'position: fixed !important; top: 0 !important; left: 0 !important;' + 
          ...
          loadingDiv.innerHTML = '<div class="v-ring-spin"></div>' + 
    ...
    function parseStreamUrl(urlStr) { 
      ...
      try { 
        ...
        log('PARSE_ERR', 'Lỗi khi parse URL Stream', e);
      } 
      return result; 
    } 
    ...
    function saveHistory(epiNum) { 
      try { 
        SmartStorage.setItem('watch_hist_' + seriesKey, JSON.stringify({ lastEpi: parseInt(epiNum, 10), time: Date.now() })); 
      } catch(e) {} 
    } 
    ...
    // === ĐÃ SỬA LỖI FETCH & CHUYỂN BẬT CHECK PROXY ===
    function fetchAndInjectEmbed(embedUrl, iframeElem, useGasProxy, callback) { 
      ...
      if (!isValidUrl(embedUrl)) {
        log('FETCH_ERR', 'URL Embed không hợp lệ, gán rỗng vào Iframe');
      ...
      function requestGAS(useCheckApi) { 
        ...
        fetch(gasTargetUrl) 
          ...
          .then(function (html) { 
            var isBlockedOr403 = !html || html.trim() === '' || html.indexOf('403') !== -1 || html.indexOf('Forbidden') !== -1 || html.indexOf('Access Denied') !== -1; 
            if (isBlockedOr403) { 
      ...
      // Mặc định gọi thử trực tiếp, nếu lỗi 403 tự động bật check=true
      requestGAS(false); 
    } 
    ...
    function injectHtmlToIframe(html, embedUrl, iframeElem) { 
      try {
        ...
        log('INJECT_ERR', 'Lỗi khi Inject HTML vào iframe', e);
        iframeElem.src = embedUrl;
      }
    } 
    ...
    function startAdWatcherOnIframe(iframeElem) { 
      ...
      var checkIframeDoc = function() { 
        try { 
          ...
          adObserver.observe(iDoc.body || iDoc.documentElement, { childList: true, subtree: true, attributes: true }); 
          bindVideoEvents(); 
    ...
    function updateEpisodeListFromCurrentServer() { 
      ...
      for (var j = 0; j < items.length; j++) { 
        ...
        episodeList.push({ name: "Tập " + epNum, num: epNum, streamUrl: item.embed }); 
      } 
    } 
    ...
    function buildUI() { 
      try { 
        ...
        var hasValidHist = savedHistoryEpi !== null && Number(currentEpisode) !== Number(savedHistoryEpi) && Number(currentEpisode) !== (Number(savedHistoryEpi) + 1); 
        if (hasValidHist) { 
          var nextEpiOfHist = Number(savedHistoryEpi) + 1; 
          histDiv.innerHTML = '<div style="font-size: 13px; color: #fff; text-align: center;">Lần trước bạn đã xem ở <b>Tập ' + savedHistoryEpi + '</b></div>' + 
    ...
    ensureDOMReady(function() { 
      try { 
        ...
        // Kiểm tra kĩ EMBED_STREAM_URL trước khi gọi parse
        ...
        if (prevHist && prevHist.lastEpi) { 
          ...
          if (Number(currentEpisode) !== Number(savedHistoryEpi) && Number(currentEpisode) !== (Number(savedHistoryEpi) + 1)) { 
        ...
        if (isValidUrl(EMBED_STREAM_URL)) {
          fetchAndInjectEmbed(EMBED_STREAM_URL, mainIframe, false, function() { 
            startAdWatcherOnIframe(mainIframe); 
          }); 
        } else {
          log('INIT_ERR', 'EMBED_STREAM_URL không hợp lệ. Dừng fetch ban đầu.');
          hideLoadingScreen();
        }

        if (isValidUrl(parsed.listEpisodesUrl)) { 
